...
2. 在 [Google AI Studio](https://ai.google.dev/) 获取免费 API Key
3. 输入 API Key 并保存
4. 开始创作！
...
```
"创建一个包含多个层次的复杂粒子系统，外层是蓝色烟雾，内层是金色闪烁粒子"
...
### AI 模式
1. **输入 API Key**: 在左侧面板输入你的 Gemini API Key
2. **描述效果**: 用自然语言描述你想要的粒子效果
3. **点击生成**: AI 会自动生成并应用配置
4. **连续调整**: 直接说出修改需求
...
**免费额度**: Google AI Studio 提供每天 **1500 次**免费请求！
...
1. **API Key 安全**: 不要分享你的 API Key
2. **请求限制**: 免费版每天 1500 次
3. **浏览器存储**: API Key 存储在本地 localStorage
4. **网络连接**: 需要稳定的网络访问 Google AI
